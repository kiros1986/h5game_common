    ...
    var _share = function (cmd, data, callbacks) {
        ...
        WeixinJSBridge.on(cmd.menu, function (argv) {
            ...
            if (callbacks.async && callbacks.ready) {
                ...
                var _data = {};
                if(cmd.menu == 'menu:share:timeline' || cmd.menu == 'menu:share:appmessage' ||
                    (cmd.menu == 'general:share' && (argv.shareTo == 'timeline' || argv.shareTo == 'friend'))){
                    _data.appid = data.appId ? data.appId : '';
                    _data.img_url = data.imgUrl;
                    _data.desc = data.desc;
                    _data.title = data.title;
                    _data.link = data.link;
                    _data.img_width = '640';
                    _data.img_height = '640';
                }else if(cmd.menu == 'menu:share:weibo'){
                    _data.content = data.desc;
                    _data.url = data.link;
                }
                // 就绪状态
                var theData = _extend(_data);
                if (!(argv && (argv.shareTo == 'favorite' || argv.scene == 'favorite') && callbacks.favorite === false)) {
                ...
                if (typeof WeixinJSBridge[key] === 'function') {
                    WeixinJSBridge[key] = function () {
                        try {
                            ...
                            var _apis = ['menu:general:share', 'menu:share:timeline', 'menu:share:appmessage', 'menu:share:weibo',
                                'menu:share:qq', 'general:share'];
                            _apis.forEach(function (menu) {
                                    runOn3rd_apis.indexOf(menu) === -1 && runOn3rd_apis.push(menu);
                                });
                            var _runOn3rd_apis = args.__runOn3rd_apis;
                            if(_runOn3rd_apis){
                                _apis.forEach(function (menu) {
                                    _runOn3rd_apis.indexOf(menu) === -1 && _runOn3rd_apis.push(menu);
                                });
                            }
                        } catch (e) {
                        }
                    ...
                    WeixinJSBridge[key].toString = function(){
                        return _WeixinJSBridge[key].toString();
                    };
                }
            });
        };
